...
public class Setting<T> implements ToXContentObject {
    ...
    public enum Property {
        /**
         * should be filtered in some api (mask password/credentials) 标记加密/认证
         */
        Filtered,

        /**
         * iff this setting can be dynamically updateable  标记可变(动态)
         */
        Dynamic,

        /**
         * mark this setting as final, not updateable even when the context is not dynamic
         * ie. Setting this property on an index scoped setting will fail update when the index is closed  标记不可变
         */
        Final,

        /**
         * mark this setting as deprecated  标记废弃
         */
        Deprecated,
    ...
    protected final Function<Settings, String> defaultValue; // setting默认值
    @Nullable
    private final Setting<T> fallbackSetting;      // 回退setting
    private final Function<String, T> parser;      // 解析器
    private final Validator<T> validator;          // setting校验
    private final EnumSet<Property> properties;    // 属性集合
    ...
    public String getDefaultRaw(Settings settings) {
        return defaultValue.apply(settings);        // 返回默认的setting的字符串形式
    }
     ...
     * @param settings a settings object for settings that has a default value depending on another setting if available
     */
    public T getDefault(Settings settings) {         // 返回默认的setting
