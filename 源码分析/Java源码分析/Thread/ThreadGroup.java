...
public class ThreadGroup implements Thread.UncaughtExceptionHandler {
    ...
    boolean daemon;                      // 是否是守护线程
    boolean vmAllowSuspension;           // 虚拟机是否允许推迟

    int nUnstartedThreads = 0;           // 未启动线程数量
    int nthreads;                        // 线程数量
    Thread threads[];                    // 所有线程

    int ngroups;                         // 线程组数量
    ThreadGroup groups[];                // 所有线程组
    ...
    private ThreadGroup() {     // called from C code
        this.name = "system";
        this.maxPriority = Thread.MAX_PRIORITY;   // 10
        this.parent = null;
    }
    ...
    public final void setMaxPriority(int pri) {
        int ngroupsSnapshot;            // 线程组个数快照
        ThreadGroup[] groupsSnapshot;   // 线程组快照
        synchronized (this) {
            checkAccess();
            if (pri < Thread.MIN_PRIORITY || pri > Thread.MAX_PRIORITY) {
                return;
            }
            maxPriority = (parent != null) ? Math.min(pri, parent.maxPriority) : pri; // 子线程组的优先级不能超出父线程组的优先级
