...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
    ...
    private int validationMode = 1;						// 校验模式 默认自动
    private boolean namespaceAware = false;				// 是否命名空间感知，如果校验的话就不需要命名空间感知
    private Class<? extends BeanDefinitionDocumentReader> documentReaderClass = DefaultBeanDefinitionDocumentReader.class;
    private ProblemReporter problemReporter = new FailFastProblemReporter();        // 快速失败问题报告器
    private ReaderEventListener eventListener = new EmptyReaderEventListener();		// 事件监听器
    private SourceExtractor sourceExtractor = new NullSourceExtractor();			// 源数据抓取器
    @Nullable
    private NamespaceHandlerResolver namespaceHandlerResolver;						// 命名空间解析器
    private DocumentLoader documentLoader = new DefaultDocumentLoader();			// xml文档加载器
    @Nullable
    private EntityResolver entityResolver;											// 实体解析器
    private ErrorHandler errorHandler;												// 异常处理器
    private final XmlValidationModeDetector validationModeDetector;					// xml校验模式探测器
    private final ThreadLocal<Set<EncodedResource>> resourcesCurrentlyBeingLoaded;	// 资源是否正在被加载
